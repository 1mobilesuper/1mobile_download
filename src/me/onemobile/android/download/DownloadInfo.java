...
import me.onemobile.android.download.Downloads.Impl;
...
public class DownloadInfo {
	...
	public static class Reader {
		...
		public void updateFromDatabase(DownloadInfo info) {
			...
			info.mMediaProviderUri = getString(info.mMediaProviderUri, Downloads.Impl.COLUMN_MEDIAPROVIDER_URI);
	...
	public boolean network_con_changed = false;
	 ...
	 */
	private String setStatusToDownloadedOrUpdated() {
		String pkg = "";
		// update hashmap
		try {
			Cursor cursor = mContext.getContentResolver().query(AppsStatusProvider.getURI(mContext), new String[] { AppsStatusProvider.PACKAGE },
					AppsStatusProvider.APPDOWNLOADINGID + "='" + mId + "'", null, null);
			if (cursor == null) {
				return pkg;
			}
			if (cursor.moveToFirst()) {
				...
				if (AppsStatusProvider.appsStatus.contains(pkg)) {
					AppsStatusProvider.appsStatus.replace(pkg, AppsStatusProvider.status_downloaded);
				} else {
					AppsStatusProvider.appsStatus.put(pkg, AppsStatusProvider.status_downloaded);
				}
			}
			cursor.close();
		} catch (Exception e) {
		...
		try {
			...
			values.put(AppsStatusProvider.PATH, mFileName);
			values.put(AppsStatusProvider.STATUS, AppsStatusProvider.status_downloaded);
			mContext.getContentResolver().update(AppsStatusProvider.getURI(mContext), values, AppsStatusProvider.APPDOWNLOADINGID + "='" + mId + "'", null);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return pkg;
	...
	private void launchInstallDialog(String pkg) {
		SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(mContext);
		if (sharedPref.getBoolean(AUTO_INSTALL, false)) {
			updateAppStatus(mContext, pkg, AppsStatusProvider.status_installing);
			if (!Util.autoInstall(mFileName)) {
				updateAppStatus(mContext, pkg, AppsStatusProvider.status_downloaded);
				installByAndroidInstaller(mFileName);
			} else {
				showAlertMsg(ResUtil.getStringId(mContext.getPackageName(), "install_success"));
			}
		} else {
			installByAndroidInstaller(mFileName);
		}
	...
	public void updateAppStatus(Context context, String pkg, int currentStatus) {
		try {
			AppsStatusProvider.appsStatus.put(pkg, currentStatus);
			ContentValues values = new ContentValues();
			values.put(AppsStatusProvider.STATUS, currentStatus);
			ContentResolver contentResolver = context.getContentResolver();
			contentResolver.update(AppsStatusProvider.getURI(context), values, AppsStatusProvider.PACKAGE + "='" + pkg + "'", null);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	private void installByAndroidInstaller(String apkpath) {
		...
		try {
			...
			Toast.makeText(mContext, ResUtil.getString(mContext, "installation_package_not_exist"), Toast.LENGTH_SHORT).show();
		}
	}

	private void showAlertMsg(final int msgSource) {
		Handler handler = new Handler(Looper.getMainLooper());
		handler.post(new Runnable() {
			public void run() {
				Toast.makeText(mContext, mContext.getString(msgSource), Toast.LENGTH_LONG).show();
			}
		});
	}
	...
	public void sendIntentIfRequested(int status) {
		if (status == Downloads.Impl.STATUS_SUCCESS) {
			String pkg = setStatusToDownloadedOrUpdated();
			launchInstallDialog(pkg);
		}
		if (mPackage == null) {
			return;
		}
	...
	private boolean isReadyToStart(long now) {
		...
		case Downloads.Impl.STATUS_WAITING_TO_RETRY:
			...
			if (network_con_changed) {
				network_con_changed = false;
				mStatus = Downloads.Impl.STATUS_WAITING_FOR_NETWORK;
				return true;
			}
	...
	public boolean isOnCache() {
		return (mDestination == Downloads.Impl.DESTINATION_CACHE_PARTITION || mDestination == Downloads.Impl.DESTINATION_CACHE_PARTITION_NOROAMING || mDestination == Downloads.Impl.DESTINATION_CACHE_PARTITION_PURGEABLE);
	}

	public Uri getMyDownloadsUri() {
		return ContentUris.withAppendedId(Downloads.get_CONTENT_URI(mContext), mId);
	}

	public Uri getAllDownloadsUri() {
		return ContentUris.withAppendedId(Downloads.get_ALL_DOWNLOADS_CONTENT_URI(mContext), mId);
	}

	public Uri getMyAppUri() {
		return ContentUris.withAppendedId(AppsStatusProvider.getURI(mContext), mId);
	}
	...
	boolean shouldScanFile() {
		return !mMediaScanned && mDestination == Downloads.Impl.DESTINATION_EXTERNAL && Downloads.Impl.isStatusSuccess(mStatus)
		/*
		 * &&
