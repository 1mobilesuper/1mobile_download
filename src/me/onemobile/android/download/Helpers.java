...
public class Helpers {
    ...
    public static class GenerateSaveFileError extends Exception {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 6961636403335511204L;
		int mStatus;
        String mMessage;
    ...
    private static File locateDestinationDirectory(Context context, String mimeType,
        ...
        return getExternalDestination(context, contentLength);
    }

    private static File getExternalDestination(Context context, long contentLength) throws GenerateSaveFileError {
    	if (DownloadThread.SWITCH_DOWNLOAD_POSITION_SDCARD) {
	        if (!isExternalMediaMounted()) {
	            throw new GenerateSaveFileError(Downloads.Impl.STATUS_DEVICE_NOT_FOUND_ERROR,
	                    "external media not mounted");
	        }
    ...
    public static final boolean discardPurgeableFiles(Context context, long targetBytes) {
        Cursor cursor = context.getContentResolver().query(
                Downloads.get_ALL_DOWNLOADS_CONTENT_URI(context),
                null,
                "( " +
        ...
        try {
            ...
            while (!cursor.isAfterLast() && totalFreed < targetBytes) {
                ...
                context.getContentResolver().delete(
                        ContentUris.withAppendedId(Downloads.get_ALL_DOWNLOADS_CONTENT_URI(context), id),
                        null, null);
                cursor.moveToNext();
            }
        } finally {
     ...
     */
    /* package */ static void deleteFile(Context ctx, ContentResolver resolver, long id, String path, String mimeType) {
        try {
        ...
        resolver.delete(Downloads.get_ALL_DOWNLOADS_CONTENT_URI(ctx), Downloads.Impl._ID + " = ? ",
