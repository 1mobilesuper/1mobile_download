...
public final class DownloadProvider extends ContentProvider {
    ...
    /** Database filename */
    private static final String DB_NAME = "omd.db";
    /** Current database version */
    private static final int DB_VERSION =191;
    /** Name of table in the database */
    private static final String DB_TABLE = "market_download";
    ...
    private static final int MY_DOWNLOADS = 100;
    ...
    private static void initAuthURI(Context ctx) {
    	AUTH = ResUtil.getString(ctx, "provider_auth_DownloadProvider");
        sURIMatcher.addURI(AUTH, "my_downloads", MY_DOWNLOADS);
        sURIMatcher.addURI(AUTH, "my_downloads/#", MY_DOWNLOADS_ID);
        sURIMatcher.addURI(AUTH, "all_downloads", ALL_DOWNLOADS);
        sURIMatcher.addURI(AUTH, "all_downloads/#", ALL_DOWNLOADS_ID);
        ...
        sURIMatcher.addURI(AUTH, "download", MY_DOWNLOADS);
        sURIMatcher.addURI(AUTH, "download/#", MY_DOWNLOADS_ID);
        sURIMatcher.addURI(AUTH,
    ...
    private final class DatabaseHelper extends SQLiteOpenHelper {
        ...
        */
		private void dropOldTables(SQLiteDatabase db) {
			try {
				db.execSQL("DROP TABLE IF EXISTS apkdownloads ");
        ...
        private void createDownloadsTable(SQLiteDatabase db) {
            try {
                ...
                db.execSQL("CREATE TABLE " + DB_TABLE + "(" +
                        ...
                        Downloads.Impl.COLUMN_IS_PUBLIC_API + " INTEGER NOT NULL DEFAULT 0 ," +
    	...
    	try {
			...
			if (dest != null) {
				if (getContext().checkCallingPermission(Downloads.Impl.PERMISSION_ACCESS_ADVANCED) != PackageManager.PERMISSION_GRANTED
						&& dest != Downloads.Impl.DESTINATION_EXTERNAL && dest != Downloads.Impl.DESTINATION_CACHE_PARTITION_PURGEABLE
			...
			if (vis == null) {
				if (Downloads.Impl.DESTINATION_EXTERNAL == dest) {
			...
			return ContentUris.withAppendedId(Downloads.get_CONTENT_URI(context), rowID);
    	} catch (Exception e) {
    		return Uri.EMPTY;
    	}
	}
    ...
    private void notifyContentChanged(final Uri uri, int uriMatch) {
		...
		if (BASE_URIS == null) {
			BASE_URIS = new Uri[] { Downloads.get_CONTENT_URI(getContext()), Downloads.get_ALL_DOWNLOADS_CONTENT_URI(getContext()), };
		}
    ...
    private void logVerboseOpenFileInfo(Uri uri, String mode) {
        ...
        Cursor cursor = query(Downloads.get_CONTENT_URI(getContext()),
